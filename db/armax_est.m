function [parameters, LL, errors, SEregression, diagnostics, VCVrobust, VCV, likelihoods, scores]=armax_est(y,constant,p,q,x,startingVals,options,holdBack,sigma2)
...
if ~isempty(startingVals) && maxq > 0
    ...
    newMAParameters = convert_ma_roots_F(MAParameters,q);
...
if maxq==0
    ...
    [Y,regressors]=newlagmatrix_F(y,maxp,constant);
    if constant
...
elseif  maxq>0 && (isempty(startingVals) || startingValsProblem)
    ...
    [Y,regressors]=newlagmatrix_F(y,max(maxp,T50),constant);
    if K>0
    ...
    while count<= 20;
        ...
        [temp,elags]=newlagmatrix_F(e,max(maxq,maxp),0); %#ok<ASGLU>
...
if maxq>0
    ...
    % parameters = fminsearch('armaxfilter_likelihood_F',parameters,optionssearch,p,q,constant,yAugmentedForMA,xAugmentedForMA,m);
end

[LL,likelihoods,errors]=armaxfilter_likelihood_F(parameters,p,q,constant,yAugmentedForMA,xAugmentedForMA,m,sigmaAugmentedForMA);
...
if nargout>=5
    ...
    if maxq==0
        [leeds,lags] = newlagmatrix_F(y,maxp,0);
        lags = lags(:,p);
        ...
        [VCVrobust,~,B,scores]=robustvcv(@armaxfilter_likelihood_F,parameters,0,p,q,constant,yAugmentedForMA,xAugmentedForMA,m,sigmaAugmentedForMA);
        VCV=B^(-1)/(T-m);
    end
end
...
function [y,x]=newlagmatrix_F(x,nlags,c)
% Construction of a matrix of lags (X) and a vector (Y) for use in an
% autoregression
%
% USAGE:
%     [Y,X]=newlagmatrix_F(Y,NLAGS,C)
%
% INPUTS:
...
%
% COMMENTS:
%     Original name, 'lagmatrix' conflicts with a Matlab file, so newlagmatrix_F
%     replaces original 
...
switch nargin
    ...
    case 3
        %Nothing
    otherwise
        error('newlagmatrix_F requires either 2 or 3 arguments.')
end
...
function [LLF, likelihoods, errors] = armaxfilter_likelihood_F(parameters,p,q,constant,y,x,m,sigma)
% PURPOSE:
%   Likelihood function for armaxfilter
%
% USAGE:
%   [LLF, LIKELIHOODS, ERRORS] = armaxfilter_likelihood_F(PARAMETERS,P,Q,CONSTANT,Y,X,M)
%   [LLF, LIKELIHOODS, ERRORS] = armaxfilter_likelihood_F(PARAMETERS,P,Q,CONSTANT,Y,X,M,SIGMA)
%
% INPUTS:
...
function parameters = convert_ma_roots_F(parameters, q)
