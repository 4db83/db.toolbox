function [beta,mle_var,results]=olsd(y,x)
% PURPOSE: least-squares regression 
%---------------------------------------------------
% USAGE: results = olsd(y,x)
...
results.beta    = xpxi*(xpyi);
beta            = xpxi*(xpyi);
results.yhat    = x*results.beta;
results.resid   = y - results.yhat;
resid           = y - results.yhat;
...
sigb            = sqrt(tmp);
e               = y - results.yhat;
ll              = -nobs/2*( (log(2*pi)+1) + log(e'*e/nobs) );

for i=1:cols(x);
    xe(:,i)=x(:,i).*e;
end;

se_white        = sqrt(diag((xpxi)\((xe)'*(xe))/(xpxi)));
results.bstd    = sigb;
