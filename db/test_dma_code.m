...
R            	= 15;							% No. of IS observations

% Required Parameter inputs 
Params.Lamda 	= .999;						% Lambda variance forgettng factor
Params.Alpha 	= .80;						% Prob. updating foregetting factor
...
for t = 1:T;
		...
		for m = 1:M;
				...
				if t==1
						...
						H(m,t)			= (1-Kappa)*(y(t-1) - yhat(m,t-1)).^2 + Kappa*H(m,t-1);
				...
				Fmt(m,:)				= xtm*Ptt_1*xtm' + H(m,t);				% DO NOT STORE ANYMORE AS NOT NEEDED.
				% Kalman Gain
				KG							= Ptt_1*xtm'/Fmt(m,:);
		 		% Updating
		 		beta_tt{m}(:,t) = btt_1 + KG*(y(t) - yhat(m,t));
		...
		pitt(:,t) = reweight(pitt_1(:,t).*norm_pdf(y(t),yhat(:,t),Fmt));
		% --------------------------------------------------------------------------------------------------
		% Timer
		if timer_on 
				if t==10				; mytimer(tt.t0,tt.t00,t,T); end;
				if mod(t,1e0)==0; mytimer(tt.t0,tt.t00,t,T); end;
		end
		% --------------------------------------------------------------------------------------------------
