...
% Volality initialisation for all models for first period
Hmt		= H_00*ones(M,1);		
% REGRESSOR INDEX NEEDED LATER.
Ix = (1:K)';					
bfull = zeros(M,K);
...
for t = 1:T;
		...
		for m = 1:M;
				...
				if t==1
						...
						Hmt(m)			= (1-Kappa)*(y(t-1) - yhatmt_1(m)).^2 + Kappa*Hmt_1(m);
				...
				eps_t						= y(t) - yhatmt(m);
		 		% Updating
		 		beta_tt{m}(:,t) = btt_1 + KG*eps_t;
				BETA_tt{m}			= btt_1 + KG*eps_t;
				Pmt{m}					= Ptt_1 - KG*xtm*Ptt_1;						% UPDATED (t|t) MSE (OR VARIANCE) OF STATES (BETA)
				% (MxK) MATRIX OF FULL BETAS
				BETA_FULL(m,Ix(S(m,:)))	= BETA_tt{m}; %beta_tt{m}(:,t);
		% END MODEL LOOP
		end;
		% TIME LOOP CONTINUES
		% --------------------------------------------------------------------------------------------------
		% MODEL PROBABILITIES
		% --------------------------------------------------------------------------------------------------
		if t > 1; pitt_1(:,t) =	Pr_hat(pitt(:,t-1),Alpha); end;				% Prediceted model Probs (pitt(:,t-1).^Alpha)./sum(pitt(:,t-1).^Alpha); for t = 1 it is pi_00.
		pitt(:,t) = reweight(pitt_1(:,t).*norm_pdf(y(t),yhatmt,Fmt));	% Updating model Probs (eq. 16 in KK) pi_{t|t-1}NormPDF(y_t,yhat_t,F^m_{t}) (MxT) dimension
		% --------------------------------------------------------------------------------------------------
		...
		yhatmt_1	= yhatmt;
		BETA_t_1  = BETA_tt';
		% IF NEEDED: store all the EWMA-TV Volatilities 
		H(:,t)		= Hmt;								% All EWMA TVP Vols
		yhat(:,t) = yhatmt;
		% --------------------------------------------------------------------------------------------------
		% CREATE DMA/DMS/TVP ETC BHATS AT EACH TIME SO THAT WE DON'T NEED TO STORE FULL beta_tt MATRIX.
		% MAKE FULL SET OF BHATs AT EACH TIME PERIOD TO BE USED TO COMPUTE DMA/DMS ETC BASED BHATS.
		[Spitt_1_t, SI_t]		= sort(pitt_1(:,t),'descend');
		BHAT.tvp(t,:)				=	BETA_tt{m};																% TVP
		BHAT.dms(t,:)				=	BETA_FULL(SI_t(1),:);											% DMS
		BHAT.dma(t,:)				= sum(repmat(pitt_1(:,t),1,K).*BETA_FULL);	% DMA
		BHAT.dma_ew(t,:)		=	mean(BETA_FULL);													% Equally weighted
		BHAT.dmq(t,:)				= sum(repmat(reweight(Spitt_1_t(SI_t(1:Qbar))),1,K).*BETA_FULL(1:Qbar,:));
		BHAT.dmq_ew(t,:)		= mean(BETA_FULL(SI_t(1:Qbar),:));
		% best_mode_t				= max(pitt_1(:,t))
		% sort the predcited model probs pi_{t|t-1}
	
		%BHAT.fullt(xindx(S(m,:)),:)		= beta_tt{m}(:,t);
		%BHAT.fullt'
		% BETA_tt{SI(1)}
		
		yhat_dma_ew(t,1)	= mean(yhatmt);
		yhat_dma(t,1)			= pitt_1(:,t)'*yhatmt;
		
		% --------------------------------------------------------------------------------------------------
		% Timer
		if timer_on 
				if t==10				; mytimer(tt.t0,tt.t00,t,T); end;
				if mod(t,5e0)==0; mytimer(tt.t0,tt.t00,t,T); end;
		end
end;
%
% bb = BHAT.dma_ew;
...
[yfit_in,mse_in,best,sorts,subset_I] = ...
	dma_combine_model_forecasts_funct(y,yhat,pitt_1,sumS,subset,Qbar);
...
for t = 1:T
	...
	for m = 1:M
		bhat.full(Ix(S(m,:)),t,m) = beta_tt{m}(:,t);
	end;
...
[yfit_oo,mse_oo,best_oo,sorts_oo,subset_I_oo] = ...
	dma_combine_model_forecasts_funct(yTh_T,yhat_oo,piTh_T,sumS,subset,Qbar);
% =================================================================================================
...
[yfit1, mse1, ~, pips1, xnames1, S1, ~] = ...
	dma_funct(y,x,h,R,PARAMS,Priors,timer_on,Qbar,always_in,no_constant);
% % % -------------------------------------------------------------------------------------------------
% % 
% % mse_in
% % mse1.in
...
subplot(2,1,2)
%plot([pips(:) pips1(:)])
% plot([BHAT.dma_ew(:) bhat.dma_ew(:)])
FF = [BHAT.dmq_ew(:) bhat.dmq_ew(:)];
disp(FF);plot(FF);
% clf
% plot(yTh_T,'r--');hold on;
% FF = [sum(bb_TT.*x_TT,2) yfit1.oo.dma];
% plot(FF(:,1),'g','Linewidth',2)
% plot(FF(:,2),'k--','Linewidth',2)
% disp(FF)
% % hold off;
% toc;

% mean((repmat(yTh_T,1,2)-FF).^2)
%%
% plot(pitt'); hold on;
