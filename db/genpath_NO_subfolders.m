% p = genpath_NO_subfolders(d, excludeDirs)

% CALL AS: p = genpath_NO_subfolders('local_functions', '_old')
% 
...
function p = genpath_NO_subfolders(d, excludeDirs)
	...
	for i=1:length(dirs)
		...
		if ~any(regexp(dirname,['^\.$|^\.\.$|^\@.*|^private$|' excludeStr ],'start'))
		  p = [p genpath_NO_subfolders(fullfile(d,dirname),excludeStr)]; % recursive calling of this function.
		end
	end
end
