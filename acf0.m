function [autocorr, sigma2_y] = acf0(aL,bL,N,sigma2_e)
% Computes the theoretical autocorrelations and long-run variance of an ARMA(p,q) process
%
% USAGE:
%  [AUTOCORR, SIGMA2_T] = acf0(aL,bL,N)
%  [AUTOCORR, SIGMA2_T] = acf0(aL,bL,N,SIGMA2_E)
% 
% INPUTS:
%  aL;				 - alpha(L) = [1 -alpha_1 -alpha_2 ... -alpha_p] polynomial terms.
%  bL;				 - beta(L)  = [1 +beta_1  +beta_2  ... +beta_q] polynomial terms.
...
% Modified by Daniel Buncic.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

% I have added this to the MFE function as I want to be able to put in the alpha(L) and
% beta(L) polynomials rather then the coefficients. Much easier to work with.
phi		= -aL(2:end);
theta = bL(2:end);
