function []= lsti(data_without_dates,decimls,unts)
% -------------------------------------------------------------------------------------
...
if isstruct(x)
	...
	if if_no_dates
		% is0 = x==0
		xout = [num2str(indx,'%d') repmat('  ',Nx,1) num2str(x(:,1:end),[prnt_format '  '])];
	else
		xout = [datestr(x(:,1), date_format) repmat('    ',Nx,1) num2str(x(:,2:end),[prnt_format '  '])];
	end;
