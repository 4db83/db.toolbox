function [paramvec,ic,s2,errors,parameters,stderr,tstat,pval,const,conststd,r2,vcv] = var_bic(y,constant,lags,het,uncorr)
%function [ic,parameters,stderr,tstat,pval,const,conststd,r2,errors,s2,paramvec,vcv] = var_bic(y,constant,lags,het,uncorr)
...
%
% USAGE:
%   [paramvec]=var_bi(Y,CONSTANT,LAGS)
%   or full estimation 
%   [paramvec,ic,s2,errors,parameters,stderr,tstat,pval,const,conststd,r2,vcv] 
%   = var_bic(y,constant,lags,het,uncorr)
%
% INPUTS:
...
%
% OUTPUTS:
%   BIC:          - Bayesina Iinformation criterion on appropriate lag order to use.
...
% adjusted sample size T
T = Ry-m;

ylags=cell(K,1);
X = zeros(Ry-m,K*sum(lags>0)+constant);
Y = y(m+1:Ry,:);
index=1;
...
paramvec = X\Y;
errors = Y-X*paramvec;
s2=errors'*errors/T;      %% MLE Sigma2 estimate, no adjustment to degrees of freedom
% VCV estimation
vcv=vectorarvcv(X,errors,het,uncorr);

aic = log(det(s2)) + (2/T)*m*K^2;
bic = log(det(s2)) + (log(T)/T)*m*K^2;
